...
impl GoExporter {
    ...
    pub fn render_node(&self, node: &AstNode) -> Result<String> {
        match node {
            ...
            AstNode::Column(Symbol::Ready(name)) => Ok(format!("CE[{}.Name()]", name)),
            AstNode::Column(Symbol::Pending(name)) => Ok(format!("??{}??", name)),
    ...
    pub fn render_funcall(&self, func: &Verb, args: &[AstNode]) -> Result<String> {
        match func {
            Verb::Builtin(builtin) => match builtin {
            ...
            Verb::Pending(name) => Ok(format!("Unknown raw function: {}", name)),
            Verb::Ready(node) => Ok(format!("Ready")),
        }
    }
}
