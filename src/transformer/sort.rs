...
fn create_sort_constraint(
    ...
    cs.constraints.push(Constraint::Vanishes {
        ...
        expr: Box::new(Intrinsic::Mul.call(&[
            Node::column().handle(eq.clone()).t(Magma::Boolean).build(),
            Intrinsic::Sub.call(&[
                Node::from_const(1),
                Node::column().handle(eq.clone()).t(Magma::Boolean).build(),
            ])?,
    ...
    for at in ats.iter() {
        cs.constraints.push(Constraint::Vanishes {
            ...
            domain: None,
            expr: Box::new(Intrinsic::Mul.call(&[
                Node::column().handle(at.clone()).t(Magma::Boolean).build(),
                Intrinsic::Sub.call(&[
                    Node::from_const(1),
                    Node::column().handle(at.clone()).t(Magma::Boolean).build(),
                ])?,
    ...
    cs.constraints.push(Constraint::Vanishes {
        ...
        expr: Box::new(
            Intrinsic::Sub.call(&[
                Node::from_const(1),
                Intrinsic::Add.call(
                    &std::iter::once(Node::column().handle(eq.clone()).t(Magma::Boolean).build())
                        .chain(
                            ats.iter()
                                .cloned()
                                .map(|at| Node::column().handle(at).t(Magma::Boolean).build()),
                        )
                        .collect::<Vec<_>>(),
                )?,
            ])?,
